...
begin
	...
	mole_fxns = [0.2, 0.8] # for [Xe, Kr] respectively
	@assert sum(mole_fxns) == pressure # quick check

	# force field: 12-6 Lennard-Jonnes potential using UFF params
	ljff = LJForceField("UFF")
	
	# COF
	xtal = Crystal("14010N2_ddec.cif")
end
...
begin
	...
	res, mols = μVT_sim(xtal, molecules, temp, mole_fxns, ljff, n_cycles_per_volume=10, write_adsorbate_snapshots=true, snapshot_frequency=100)
end
...
begin
	...
	###
	write_xyz(Frac.(mols[1], xtal.box), xtal.box, joinpath(rc[:paths][:data], "snapshots", "mVT_Xe_in_14010N2.xyz"))

	write_xyz(Frac.(mols[2], xtal.box), xtal.box, joinpath(rc[:paths][:data], "snapshots", "mVT_Kr_in_14010N2.xyz"))
end
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "ef5149943686407d7ef50314cfe6dd15dd44b07e"
...
git-tree-sha1 = "b2735da3f9fd6d6787d1a7e354682a7a717ba897"
